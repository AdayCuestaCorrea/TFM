...
Toda cifra citada aquí procede directamente de \texttt{src/\allowbreak{}evaluation/\allowbreak{}metrics.py} a través
de \texttt{report\_tables.py}; ninguna se teclea a mano, y cada tabla llega al documento
mediante \texttt{\textbackslash input\{tables/\allowbreak{}...\}} desde el fichero que
\texttt{export\_latex\_tables.py} genera a partir de los artefactos persistidos por las
fases 3 a 5b. La respuesta al contraste central del TFM (\ref{sec:contraste_central}) se
...
persistencia simple es un listón trivial en esta serie, y el resultado interesante de este
...
\begin{figure}[htbp]
  ...
  \caption{\headlinecolor{Comparación de modelos en la partición test. Barras agrupadas de MAE, RMSE, MAPE y R², ordenadas de mejor a peor por MAE. El color codifica la familia del modelo.}}
...
si fuera una estimación honesta de desempeño: train es, por construcción, una muestra
...
partición de test; la de validación reproduce el mismo patrón. El ajuste visual confirma lo que las
tablas de la sección anterior ya cuantificaban: la predicción sigue de cerca tanto la
oscilación semanal (el diente de sierra laborable/fin de semana identificado en la
sección \ref{sec:eda}) como las transiciones asociadas a festivos aislados, sin el retraso
de fase que delataría un modelo que en realidad estuviera prediciendo el valor del día
anterior en lugar del día en curso. Las mayores desviaciones puntuales, visibles como
picos aislados entre la serie real y la predicha, se concentran en los días de calendario
irregular, festivos y puentes, cuyo comportamiento cuantitativo se desglosa con detalle en
la sección \ref{sec:error_dia}; el resto de la serie, que constituye la inmensa mayoría de
las observaciones, se sigue con una fidelidad que resulta coherente con el MAPE de test
del 3,13\,\% reportado en la tabla \ref{tab:comparacion_test}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.78\textwidth]{demand_actual_vs_predicted_test.png}
  \caption{\headlinecolor{Demanda diaria real y predicha por el modelo ensemble\_equal en la partición test. Serie observada y predicha superpuestas.}}
...
prácticamente unitario y un error de test sustancial es la evidencia de que el modelo
...
\begin{figure}[htbp]
  ...
  \caption{\headlinecolor{Distribución del error de predicción del modelo xgboost\_alone en la partición test. La brecha train-test es la evidencia visual del sobreajuste (R² train=0,9929 vs. MAE test=156.121).}}
...
fuerza aquí una comparación que los datos disponibles no permiten hacer honestamente. La
...
\begin{figure}[htbp]
  ...
  \caption{\headlinecolor{Distribución del error de predicción del modelo hybrid en la partición test.}}
...
dos componentes individuales por separado. Este estrechamiento no es un artefacto
cosmético, sino la manifestación directa, en la distribución del error, del mecanismo de
reducción de varianza por descorrelación de errores que se cuantifica formalmente en la
...
\begin{figure}[htbp]
  ...
  \caption{\headlinecolor{Distribución del error de predicción del modelo ensemble\_equal (ganador) en la partición test.}}
...
\begin{figure}[htbp]
  ...
  \caption{\headlinecolor{Importancia agregada por grupo de variables del modelo xgboost\_alone, medida como ganancia.}}
...
\begin{table}[htbp]
  ...
  \caption{\headlinecolor{Importancia por grupo de variables, xgboost\_alone.}}
...
\begin{table}[htbp]
  ...
  \caption{\headlinecolor{Top-15 variables individuales por ganancia, xgboost\_alone.}}
...
sugerida por un ranking de importancia, no un resultado medido, y que las fases
...
\begin{figure}[htbp]
  ...
  \caption{\headlinecolor{Importancia agregada por grupo de variables del modelo xgboost\_\allowbreak{}residual (etapa 2 del híbrido).}}
...
\begin{table}[htbp]
  ...
  \caption{\headlinecolor{Importancia por grupo de variables, xgboost\_residual.}}
...
segunda variable de la lista. Esto confirma, al nivel más granular posible, que la etapa 2
sí aprende exactamente la señal de calendario (en concreto, la irregularidad de los
festivos) que la etapa 1 univariante no puede ver por construcción: el mecanismo que
justifica teóricamente la arquitectura híbrida es real, medible y localizado en la
variable exacta que cabría esperar, con independencia de que el veredicto agregado de la
...
\begin{table}[htbp]
  ...
  \caption{\headlinecolor{Top-15 variables individuales por ganancia, xgboost\_residual. \texttt{feat\_\allowbreak{}day\_\allowbreak{}type\_\allowbreak{}festivo} encabeza con ganancia 0,125 -- confirma que la etapa 2 aprende la señal de calendario que la etapa 1 no puede ver.}}
...
forma explícita e inequívoca a la pregunta que estructura todo el proyecto: ¿mejora la
...
directamente sobre las mismas variables exógenas? La respuesta, sostenida por la tabla
...
\begin{table}[htbp]
  ...
  \caption{\headlinecolor{Diferencia de MAE del híbrido frente a SARIMAX y XGBoost-alone (Delta MAE = hybrid - competidor; positivo = híbrido peor). Generada, no tecleada a mano.}}
...
aprendizaje automático que utiliza exactamente la misma información exógena sin pasar por
ninguna etapa LSTM. Este segundo resultado es, si cabe, el más informativo de los dos: si
...
este resultado, en lugar de limitarse a documentarlo. Granger \cite{granger1989combining},
...
\emph{y}, de manera crucial, se basan en conjuntos de información distintos entre sí. Si
dos modelos observan exactamente la misma información y solo difieren en la forma
funcional con la que la procesan, combinarlos secuencialmente no introduce nada que uno de
los dos no pudiera ya haber aprendido por sí mismo con una arquitectura suficientemente
flexible.

La arquitectura híbrida de este TFM \textbf{incumple precisamente esa segunda condición}. Ambas
etapas observan, en última instancia, el mismo objeto: el pasado de la serie
\texttt{total}. La etapa 1 lo observa directamente, en su forma univariante y sin
calendario; la etapa 2 lo observa de forma indirecta, a través de un residuo que es
función de ese mismo pasado más un conjunto de variables exógenas que, como muestra la
sección \ref{sec:importancia_variables}, sí aportan información adicional genuina —de
ahí que la etapa 2 mejore de forma sustancial a su propia etapa 1—, pero que no
constituyen un conjunto de información \emph{independiente} de la demanda pasada, sino un
complemento sobre la misma serie. El resultado es exactamente el que la teoría de Granger
predeciría: una mejora real y medible de la etapa 2 sobre la etapa 1 (documentada en la
sección \ref{sec:error_dia} y de nuevo en la \ref{sec:importancia_variables}), pero
insuficiente para que la combinación secuencial supere a un modelo de una sola etapa que
parte directamente de una base más sólida: SARIMAX, con su propio mecanismo
autorregresivo y estacional, o \texttt{xgboost\_alone}, que accede a las mismas 161
variables exógenas sin arrastrar el ruido de una etapa 1 estructuralmente débil.
...
problema completo desde el principio con la información exógena disponible. Este patrón
—hibridar puede mejorar sustancialmente a un componente débil sin llegar a igualar a un
componente fuerte de una sola etapa cuando ambos comparten información— no es exclusivo de
este proyecto: es consistente con lo observado de forma sistemática en las competiciones
de pronóstico a gran escala M4 y M5 \cite{makridakis2018m4,makridakis2022m5}, donde los métodos híbridos y
...
sección \ref{sec:mecanismo_ensemble} demuestra para el ensamble de este TFM, y no al
encadenar dos etapas sucesivas sobre el mismo conjunto de información. El resultado
...
\begin{table}[htbp]
  ...
  \caption{\headlinecolor{Veredicto del criterio pre-registrado del experimento A (repesado 3x de días irregulares). CRITERIO NO CUMPLIDO: el presupuesto global se cumplió pero el error en días irregulares NO mejoró.}}
...
peso adicional no parece comprar un mejor ajuste específicamente en esas filas; en su
...
\subsection{Experimento B: ensamble SARIMAX + XGBoost-alone}\label{sec:por_que_ensemble}

El segundo experimento no está sujeto a un criterio de éxito binario, sino que constituye
un punto de comparación que cualquier evaluación honesta de este proyecto debe incluir:
...
competitivas, sino que \textbf{superan a los dos componentes individuales}, y lo hacen por
un margen que una simple media de errores independientes no explicaría por sí sola (el
mecanismo exacto de esa ganancia se desarrolla en la sección
\ref{sec:mecanismo_ensemble}). Los dos esquemas de ponderación producen resultados casi
...
\begin{figure}[htbp]
  ...
  \caption{\headlinecolor{Dispersión de residuos SARIMAX vs. XGBoost-alone en la partición test -- evidencia mecánica de la reducción de varianza del ensamble (correlación débil entre ambos errores).}}
...
correlación perfecta es, precisamente, la que hace mecánicamente rentable promediar los
...
\cite{bates1969combination}: con $\rho = 1$, errores perfectamente correlacionados,
...
\cite{perrone1993networks}, aplicado aquí no a una colección de redes con la misma
...
(\texttt{xgboost\_alone}, 156.121), es mayor de lo que un simple promedio de errores
sin ninguna estructura común explicaría, lo que confirma que la ganancia procede de esta
descorrelación genuina y no de un artefacto de redondeo en la comparación.
...
estacionalidad semanal. La cuota de potencia espectral concentrada exactamente en el bin
...
existe un bin de Fourier exactamente en 7,0 días, y la potencia semanal se reparte entre
los bins adyacentes (6,4 y 6,7 días) y, sobre todo, hacia el armónico de 7/3 días, que es
...
real que la prueba de bin único infravalora.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.78\textwidth]{fig_periodograma_residuo_lstm.png}
  \caption{\headlinecolor{Periodograma del residuo de la etapa 1: cuota de potencia espectral frente al periodo en días (escala logarítmica). El periodo de 7 días está anotado; la potencia semanal se dispersa entre bins adyacentes y el armónico de 7/3 días.}}
...
Donde el residuo sí retiene estructura de forma inequívoca es en el calendario irregular
...
el único de los cuatro criterios de aceptación pre-registrados que cae con claridad en el
...
híbrido pierde. Lo que este análisis descarta es que quedara sobre la mesa una palanca
exógena meteorológica sin explotar; lo que el residuo retiene es estructura de calendario,
...
distancia entre una LSTM univariante y un XGBoost directo cierra realmente la etapa
...
un XGBoost directo sobre las mismas 161 variables. Sobre validación la corrección es
...
La evidencia decisiva sobre el papel del tamaño de muestra procede de una curva de
...
con la misma prominencia que tendría un resultado positivo. Por un lado, el tamaño de
...
Por otro lado, y esto es lo que la curva zanja, esa meseta se sitúa muy por encima de ambas
...
segunda (Q2): ¿integra ya el XGBoost directo los efectos temporales y exógenos de forma
...
una constante de módulo evaluada en código, no a ojo. El umbral del 5\,\% absorbe el ruido
...
prueba nueva; el capítulo ya demostró la mitad predictiva, que el híbrido pierde): la
...
pueda leerse de un vistazo junto con su estabilidad entre particiones.
...
de 28 pasos del objetivo alimenta la LSTM y el vector de variables exógenas \texttt{feat\_}
...
Cada columna exógena es la misma fila limpia de fuga que ve XGBoost (guardas de las fases 2
y 3: sin operadores del mismo día, sin meteorología del mismo día), y una aserción
...
es una conclusión fuerte} (queda descartada la asimetría informativa como causa) y
\textbf{una victoria es débil} (procede en parte de información extra, no solo de la
arquitectura). Y un buen resultado aquí sería un argumento \emph{contra} el híbrido, no a
favor de uno mejor: una etapa 1 que ya ve las variables exógenas no deja a la etapa 2 nada
...
semilla individual (411.142, la cifra citada en la sección \ref{sec:respuesta_preguntas} y
en el capítulo \ref{cap:metodologia}). El veredicto de paridad se lee sobre una mediana de
...
regla preregistrada (secciones de método y objeción O6: subir a cinco solo si la sonda de
...
\textbf{Lecturas preregistradas.} Con un hueco de 171.481, cerrar el $\geq$ 60\,\%
...
ser univariante: la paridad informativa no la rescata.
...
\texttt{xgboost\_alone} ya usa de forma demostrable tanto la información temporal como la
...
exactamente las variables exógenas que ve XGBoost, e incluso más información en forma de
ventana cruda, la vía recurrente no alcanza a los modelos de una sola etapa. La debilidad
de la etapa 1, por tanto, no es un problema de acceso a la información que la etapa 2
compense: es la combinación de una arquitectura recurrente y unas 860 observaciones diarias.
...
único modelo de \emph{gradient boosting} sobre variables exógenas bien construidas supera a
un híbrido residual secuencial, y el beneficio del híbrido se limita a rescatar una etapa
temporal débil en lugar de a batir a modelos de una sola etapa fuertes.

La ablación permite además cerrar una tensión que arrastran las secciones anteriores y que
un lector atento habrá advertido. Tres hechos sobre la meteorología retardada parecen
...
anómala. Esta sección la responde de forma sistemática y honesta. Bajo el mismo régimen de
...
El riesgo dominante aquí no es la incompletitud, es el \emph{p-hacking}: buscar subgrupos
hasta encontrar uno que favorezca al híbrido es \emph{p-hacking} con independencia de por
qué se emprenda la búsqueda. Por eso el diseño se fijó \textbf{antes} de calcular ningún MAE, ranking
o diferencia —solo se contaron los tamaños muestrales—, de modo que ``el híbrido no gana en
ningún subgrupo'' sea tan publicable como cualquier hallazgo positivo.

\subsection{Diseño preregistrado}\label{sec:desglose_diseno}
...
(393 días contiguos) es un ámbito \textbf{secundario} preregistrado, usado solo de forma
...
, el único fijado por nombre en el diseño preregistrado, tiene $n=16$ en test y $n=34$ en
...
la diferencia entre ambos y el veredicto preregistrado. La figura
...
El resultado es inequívoco. De los quince subgrupos preregistrados, nueve alcanzan $n \geq
...
que es no inferencial y además no replica el hecho en test. La búsqueda está hecha y su
...
reporte a posteriori, pero \textbf{ilegítima como regla de enrutamiento}. Ningún lector
debe concluir ``despliéguese el híbrido en los días anómalos'': la anomalía no se conoce
...
La incertidumbre de cada diferencia se obtiene con un \emph{bootstrap} pareado de bloques
...
dependencia semanal, que es la autocorrelación dominante y la razón por la que un test
...
así que los tests son positivamente dependientes —el régimen (PRDS) bajo el que
...
que empate. Este es el desenlace más expuesto a sobrelectura, así que su tratamiento está
preregistrado. Bajo la hipótesis nula los once modelos son intercambiables dentro de un
subgrupo, de modo que la probabilidad de que el MAE más bajo corresponda a la familia
híbrida de dos miembros es $2/11$; sobre los nueve subgrupos inferenciales eso da
$9 \times 2/11 \approx 1{,}6$ subgrupos que quedarían primeros por puro azar. El recuento
observado es 0, por debajo incluso de esa expectativa nula. Donde la inferencia es
imposible —festivo ($n=5$), laborable en puente ($n=3$)— no se fabrica ningún valor p:
se da un MAE puntual con su $n$ y nada más, que es la respuesta honesta.
...
ventanas consecutivas se solapan en 27 días, así que no se le aplica ningún test y las
ventanas que cruzan la frontera mezclan las dos particiones.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.78\textwidth]{fig_mae_movil_periodo.png}
  \caption{\headlinecolor{MAE móvil a 28 días por modelo (panel superior) y diferencia móvil mejor híbrido $-$ mejor competidor (panel inferior) sobre la ventana contigua de validación y test. Regla vertical en la frontera val/test. Figura descriptiva: sin test, por el fuerte solapamiento de ventanas consecutivas.}}
...
\begin{description}[nosep,leftmargin=0pt,labelindent=0pt,style=nextline,font=\bfseries]
  ...
  \item[PI4. ¿Funciona la etapa 2 como mecanismo de corrección residual, aunque el
    ...
    (0,125), confirma a nivel de variable individual que esa corrección procede exactamente
  ...
  \item[PI5. ¿Existe una combinación más simple que supere a ambos componentes de una
    ...
    un 10,5\,\% por debajo del mejor de sus dos componentes por descorrelación de errores
    (sección \ref{sec:mecanismo_ensemble}).
\end{description}
...
control de paridad informativa (\ref{sec:paridad_lstm}) descarta que la etapa 1 pierda por
ser univariante: sigue por detrás de los modelos de una sola etapa incluso viendo las
mismas variables exógenas que XGBoost, con veredictos de \emph{limitación arquitectónica}
(\texttt{calendario}) y \emph{mixto/ambiguo} (\texttt{completo}).
