...
métricas: a dos decimales, el ensamble 50/50 (0,9755), XGBoost independiente
(0,9690) y SARIMAX (0,9672) colapsarían en 0,98/0,97/0,97, borrando exactamente
la distinción que esta comparación existe para establecer. El MAPE, en cambio, no sufre
ese problema a esta escala y se mantiene a dos decimales. En todo el capítulo, el MAE, el
...
SARIMAX y XGBoost independiente, sin entrenamiento adicional) lideran ambas
particiones: el ensamble inverso al MAE obtiene el mejor MAE de test, \textbf{139.681}, con un
$R^2$ de 0,9755, seguido a menos de un 0,05\,\% de distancia por el ensamble 50/50
(139.725). En segundo lugar, los dos modelos de una sola etapa ocupan el siguiente escalón
sin solaparse con el anterior: XGBoost independiente (MAE test 156.121, $R^2=0{,}9690$) y
...
lugar, la familia LSTM (híbrido ponderado, híbrido y LSTM independiente,
...
$R^2$ próximos a cero o directamente negativos, lo que apoya sobre el conjunto completo de
...
total superpuesta a la predicción del modelo de referencia, el ensamble 50/50, sobre la
...
El caso más claro de esta distinción en todo el proyecto es XGBoost independiente: su
...
error del híbrido en test sin más: una distribución visiblemente más dispersa que
la de XGBoost independiente, coherente con el MAE de test casi 50\,\% mayor que se
...
distribución de error del modelo de referencia, el ensamble 50/50: una distribución
...
partición test, SARIMAX es claramente mejor que XGBoost independiente en los días
...
que son precisamente laborables ordinarios. XGBoost independiente, en cambio, es
...
LSTM independiente de la tabla \ref{tab:desglose_dia_test}) a 666.572, una mejora del
...
XGBoost independiente (251.553): la corrección funciona, pero parte de una etapa 1
...
XGBoost independiente en 78.103; sobre validación las diferencias son menores en
magnitud absoluta, pero mantienen el mismo signo, 7.270 y 35.895 respectivamente. La
diferencia frente a XGBoost independiente es, en ambas particiones, mayor que la
...
el del otro, la combinación pierde esa fuente de ganancia, y lo que aporte dependerá de
la forma funcional de cada etapa, no de información nueva.

La arquitectura híbrida de este TFM \textbf{no satisface plenamente esa segunda condición}, y
...
variables predictoras sin arrastrar el ruido de una etapa 1 estructuralmente débil. Aunque la segunda etapa incorpora información adicional de
...
acotado en el desempeño global. El híbrido ponderado, variante descrita en la
...
aritmética las predicciones de SARIMAX y de XGBoost independiente ya calculadas en las
...
muy próximos a un reparto igualitario. Por ello, y pese a que el ensamble inverso al MAE
obtiene el MAE de test nominalmente más bajo de todo el proyecto, se adopta
el ensamble 50/50 como modelo de referencia para la memoria: ofrece un
...
Debe constar con exactitud cómo se tomó esta decisión. No existía una regla previa para
...
la relación entre los errores de SARIMAX y de XGBoost independiente en cada día de la
...
XGBoost independiente no cometen simplemente errores de magnitud distinta sobre los
...
el ensamble inverso al MAE, un 10,5\,\% por debajo del mejor componente individual
(XGBoost independiente, 156.121), es coherente con el mecanismo de reducción de varianza
...
vía independiente: la ablación de bloques (\ref{sec:ablacion_bloques}) indica
