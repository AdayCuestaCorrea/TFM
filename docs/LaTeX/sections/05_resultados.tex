...
las particiones de validación y test de la sección \ref{sec:particiones_fuga}. Ninguna
cifra se introduce manualmente: todas llegan desde los artefactos persistidos por las
fases 3 a 5b, por la cadena que detalla el Anexo \ref{cap:anexo3}.
...
la familia LSTM y dos de ensamble) ordenados de mejor a peor por MAE sobre test y
validación respectivamente. El coeficiente de determinación se reporta a cuatro decimales
y no a dos, una asimetría deliberada frente al resto de métricas: a dos decimales, el
ensamble 50/50 (0,9755), XGBoost independiente (0,9690) y SARIMAX (0,9672) colapsarían en
0,98/0,97/0,97, borrando exactamente la distinción que esta comparación existe para
establecer. El MAPE no sufre ese problema a esta escala y se mantiene a dos decimales. En todo el capítulo, el MAE, el
RMSE y sus diferencias se expresan en \textbf{viajes por día} (validaciones diarias
agregadas del CRTM, la unidad de \texttt{total}), y las cifras siguen la convención
española: el punto separa millares y la coma los decimales, de modo que 139.681 son
ciento treinta y nueve mil seiscientos ochenta y un viajes diarios.
...
La jerarquía resultante es nítida y consistente entre ambas particiones, como muestra
la figura \ref{fig:model_comparison_bar_test}. Las dos variantes de ensamble
(combinaciones puramente aritméticas de SARIMAX y XGBoost independiente, sin
entrenamiento adicional) lideran ambas particiones: el ensamble inverso al MAE obtiene el
mejor MAE de test, \textbf{139.681}, con un $R^2$ de 0,9755, seguido a menos de un
0,05\,\% por el ensamble 50/50 (139.725). Los dos modelos de una sola etapa ocupan el
siguiente escalón sin solaparse con el anterior: XGBoost independiente (MAE test 156.121,
$R^2=0{,}9690$) y SARIMAX (MAE test 163.627, $R^2=0{,}9672$) mejoran de forma clara a toda
la familia LSTM, pero quedan por detrás del ensamble en aproximadamente un 11\,\% y un
17\,\% de MAE respectivamente. La familia LSTM (híbrido ponderado, híbrido y LSTM
independiente, en ese orden) se sitúa sistemáticamente peor que ambos modelos de una sola
etapa, resultado que la sección \ref{sec:contraste_central} analiza en detalle.

Por último, los cuatro baselines ingenuos colapsan frente a cualquiera de los anteriores:
la persistencia estacional, el mejor de ellos, supera en torno a un 10\,\% en MAE al peor
miembro de la familia LSTM, y la persistencia simple y las dos medias móviles presentan
$R^2$ próximos a cero o negativos, lo que apoya sobre el conjunto completo de modelos la
advertencia del capítulo \ref{cap:metodologia}: superar a la persistencia simple es un
umbral trivial en esta serie, y el resultado interesante de este capítulo se juega
enteramente entre los siete modelos restantes.
...
Ninguna cifra de entrenamiento se presenta junto a estas dos tablas como si fuera una
estimación válida de desempeño: train es, por construcción, una muestra \emph{in-sample}
para todo modelo que se ajusta sobre ella, y su inclusión induciría a compararla
incorrectamente con val o test, un error que la sección \ref{sec:analisis_residuos} evita
explícitamente al tratar el desempeño en train como diagnóstico de sobreajuste y no como
un resultado más de la tabla maestra.

\section{Ajuste temporal}\label{sec:ajuste_temporal}

La figura \ref{fig:demand_actual_vs_predicted_test} superpone la serie real de demanda
total a la predicción del modelo de referencia, el ensamble 50/50, sobre test; validación
reproduce el mismo patrón. La predicción sigue de cerca tanto la oscilación semanal (el
diente de sierra laborable/fin de semana de la sección \ref{sec:eda}) como las
transiciones asociadas a festivos aislados, sin el retraso de fase que delataría un modelo
que en realidad predijera el valor del día anterior. Las mayores desviaciones puntuales se
concentran en los días de calendario irregular, festivos y puentes, desglosados en la
sección \ref{sec:error_dia}; el resto de la serie se sigue con una fidelidad coherente con
el MAPE de test del 3,13\,\% de la tabla \ref{tab:comparacion_test}.
...
El análisis de residuos distingue un modelo que generaliza de uno que memoriza. El caso
más claro del proyecto es XGBoost independiente: su $R^2$ de entrenamiento alcanza 0,9929,
un ajuste casi perfecto, frente a un MAE de test de 156.121; esa brecha entre un $R^2$ de
entrenamiento prácticamente unitario y un error de test sustancial es indicativa de que el
modelo memoriza una parte relevante de su entrenamiento en lugar de limitarse a capturar
la estructura genuinamente generalizable de la demanda. La figura
\ref{fig:residual_distribution_xgboost_alone} la hace visible comparando las
...
diarias. La cifra que debe citarse sobre el desempeño de este modelo es siempre la de
test, nunca la de entrenamiento como si fuera indicativa de generalización.
...
Los modelos derivados de la etapa 1 LSTM, el híbrido entre ellos, no disponen de esta
comparación train-test, puesto que la fase 4 solo persistió predicciones del modelo final
sobre validación y test, no sobre entrenamiento; no se fuerza una comparación que los
datos disponibles no permiten. La figura \ref{fig:residual_distribution_hybrid} muestra,
en su lugar, la distribución del error del híbrido en test sin más: visiblemente más
dispersa que la de XGBoost independiente, coherente con el MAE de test casi 50\,\% mayor
cuantificado en la sección \ref{sec:tabla_maestra}.
...
distribución de error del modelo de referencia, el ensamble 50/50: sensiblemente más
estrecha y mejor centrada en torno a cero que la de cualquiera de sus dos componentes por
separado. Este estrechamiento es coherente con el mecanismo de reducción de varianza por
descorrelación de errores que cuantifica la sección \ref{sec:mecanismo_ensemble}:
promediar dos modelos cuyos errores no están perfectamente correlacionados produce un
error agregado con menor dispersión que cualquiera de los dos originales, sin que ninguno
de los componentes cambie en absoluto.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.78\textwidth]{residual_distribution_ensemble_equal.png}
  \caption{\headlinecolor{Distribución del error de predicción del modelo \texttt{ensemble\_equal} (modelo de referencia) en la partición test.}}
...
domingo y festivo), y la figura \ref{fig:error_by_day_type_test} lo representa, atenuando
visualmente los grupos con menos de diez observaciones y anotando su tamaño muestral bajo
cada barra. La sección \ref{sec:desglose_subgrupos} retoma esta descomposición de forma
sistemática (quince subgrupos de calendario y periodo fijados por adelantado, con réplica
en validación y corrección por comparaciones múltiples) para responder a si el híbrido
mejora en algún régimen concreto.

\begin{table}[htbp]
  \centering
  \small
  \input{tables/tabla_desglose_dia_test}
  \caption{\headlinecolor{MAE por tipo de día (viajes por día), partición test. n=5 (festivo) y n=3 (laborable, puente): indicativos, no concluyentes. La fila ``Laborable'' ($n=136$) es la suma de ``Laborable ordinario'' (133) y ``Laborable, puente'' (3); no es un grupo adicional independiente.}}
...
\begin{table}[htbp]
  ...
  \caption{\headlinecolor{MAE por tipo de día (viajes por día), partición val. La fila ``Laborable'' ($n=133$) es la suma de ``Laborable ordinario'' (121) y ``Laborable, puente'' (12); no es un grupo adicional independiente.}}
  \label{tab:desglose_dia_val}
\end{table}

Esta descomposición revela la razón mecánica por la que el ensamble supera a los dos
modelos de una sola etapa que lo componen, y no solo el hecho de que lo hace. Sobre test,
SARIMAX es claramente mejor que XGBoost independiente en laborables ordinarios (MAE
148.297 frente a 169.488): su componente autorregresivo estacional captura con precisión
la dinámica de la inmensa mayoría de los días del año, que son precisamente laborables
ordinarios. XGBoost independiente, en cambio, es netamente superior en fin de semana: en
domingo su MAE es 100.200 frente a los 169.129 de SARIMAX, y en sábado 129.381 frente a
203.910, casi 40\,\% y 37\,\% mejor, respectivamente. \textbf{Ningún modelo domina al otro de forma global}; cada uno acierta mejor en
...
única excepción de la comparación frente a LSTM independiente, peores que SARIMAX y que
XGBoost independiente en prácticamente todos los grupos de la tabla
\ref{tab:desglose_dia_test}. El hallazgo parcialmente positivo, y el único genuinamente
favorable a la arquitectura híbrida en todo el desglose, aparece en festivos: la etapa 2
reduce el error de su propia etapa 1 de 1.585.388 (MAE de LSTM independiente en los
cinco festivos de test, obtenido con la misma descomposición por tipo de día que la tabla
\ref{tab:desglose_dia_test}, que no incluye esa columna) a 666.572 (fila festivo, columna
\texttt{hybrid} de esa tabla), una mejora del 58\,\%, que muestra de nuevo que el mecanismo de corrección funciona como está diseñado. Aun así,
ese 666.572 sigue siendo 2,5 veces peor que el MAE de SARIMAX en el mismo grupo (271.410)
y notablemente peor que XGBoost independiente (251.553): la corrección funciona, pero
parte de una etapa 1 demasiado débil para que el resultado corregido alcance a los modelos
de una sola etapa.
...
Toda esta lectura exige una nota de cautela obligatoria: en la partición test, el grupo
de festivos contiene únicamente cinco observaciones y el de laborables con puente solo
tres. Con muestras de ese tamaño, un único día atípico puede
...
ganancia de cada uno de los dos modelos XGBoost por grupo temático de variable, y las
tablas \ref{tab:top15_alone} y \ref{tab:top15_residual} descienden a las quince variables
individuales más influyentes de cada modelo. El contraste entre ambos es la evidencia más
directa del capítulo sobre qué tipo de información resuelve cada problema de
predicción.
...
En XGBoost independiente, el grupo de calendario, solo 11 de las 161 variables,
concentra el 58,5\,\% de la ganancia total (tabla \ref{tab:importancia_grupo_alone}), y
su variable más influyente con diferencia, \texttt{feat\_day\_type\_laborable}, aporta por
sí sola una ganancia de 0,561 (tabla \ref{tab:top15_alone}): al predecir la demanda
directamente, saber si el día es laborable resuelve la práctica totalidad de la varianza
semanal identificada en la sección \ref{sec:eda}, y el resto de variables (retardos de
operador, meteorología retardada, términos de Fourier) se reparten un papel claramente
secundario de matiz fino sobre ese nivel base.
...
jerarquía (figura \ref{fig:feature_importance_group_xgboost_residual}, tabla
...
coherente con la definición de la variable objetivo de esta etapa: el residuo
$e_t = y_t - \hat y^{\text{LSTM}}_t$ ya no contendría la componente de calendario más
gruesa, la distinción laborable/fin de semana, porque una parte relevante de ella ya
habría sido absorbida, con mayor o menor acierto, por la dinámica temporal de la etapa 1;
lo que quedaría por explicar sería, en consecuencia, un matiz más fino, más ligado a las
condiciones meteorológicas retardadas que al calendario grueso.
...
importante de la etapa 2, visible únicamente al nivel de variable individual (tabla
\ref{tab:top15_residual}): pese a que el calendario pesa menos como grupo, la variable con
mayor ganancia en todo el modelo residual sigue siendo \texttt{feat\_day\_type\_festivo},
con 0,125, muy por delante de la segunda de la lista. Esto es coherente, al nivel de
variable individual, con que la etapa 2 aprenda la señal de calendario (en concreto, la
irregularidad de los festivos) que la etapa 1 univariante no puede ver por construcción:
el mecanismo que justifica teóricamente la arquitectura híbrida se manifiesta en la
variable que cabría esperar, con independencia de que el veredicto agregado de la sección
\ref{sec:contraste_central} termine siendo negativo para la arquitectura completa.
...
Con la evidencia anterior establecida, esta sección responde de forma explícita a la
pregunta que estructura todo el proyecto: ¿mejora la arquitectura híbrida residual
LSTM-XGBoost, tal como se ha diseñado e implementado en este TFM, la predicción de demanda
frente a SARIMAX y frente a un XGBoost entrenado directamente sobre las mismas variables
predictoras? La respuesta, sostenida por la tabla \ref{tab:delta_hibrido}, es \textbf{no,
en ninguna de las dos comparaciones y en ninguna de las dos particiones de evaluación}.
...
Sobre test, el híbrido supera en MAE a SARIMAX en 70.597 viajes diarios y a XGBoost
independiente en 78.103; sobre validación las diferencias son menores en magnitud
absoluta, pero mantienen el signo, 7.270 y 35.895 respectivamente. La diferencia frente a
XGBoost independiente es, en ambas particiones, mayor que frente a SARIMAX: el híbrido no
solo pierde frente al baseline estadístico clásico, sino por un margen todavía más amplio
frente a un modelo de aprendizaje automático que utiliza exactamente la misma matriz de
variables predictoras sin pasar por ninguna etapa LSTM. Este segundo resultado es el más
informativo: si el problema fuera únicamente que SARIMAX está particularmente bien
ajustado a esta serie, cabría esperar que el híbrido, con acceso a variables no lineales y
a una capa de aprendizaje profundo, cerrara al menos parte de esa distancia frente a otro
modelo de aprendizaje automático; en su lugar, la distancia se amplía.

\subsection{Por qué pierde el híbrido: lectura a la luz de Granger (1989)}\label{sec:resultado_negativo_teoria}

La literatura clásica de combinación de pronósticos ofrece un marco para interpretar
este resultado, no solo documentarlo. \textcite{granger1989combining}, revisando dos
décadas de literatura sobre combinación de modelos, señala que una combinación tiende a
beneficiarse cuando los modelos combinados son individualmente subóptimos y aportan
información o errores parcialmente complementarios, lo que suele ocurrir cuando se
apoyan en información distinta. Bajo esa lectura, una parte de la
ganancia de combinar procede de información que cada componente tiene y el otro no; si el
conjunto de información de uno está contenido en el del otro, esa fuente de ganancia se
debilita, y lo que aporte la combinación dependerá sobre todo de la forma funcional de
cada etapa.

La arquitectura híbrida de este TFM \textbf{ofrece poca complementariedad en ese sentido},
y conviene precisar por qué. Los conjuntos de información de las dos etapas no son
...
tampoco son \emph{distintos} en el sentido que ese marco favorece: el pasado reciente de la
...
comparación relevante es con el mejor modelo de una sola etapa sobre el conjunto mayor, y
ese modelo existe en la comparación: XGBoost independiente, que accede a las mismas 161
variables predictoras sin arrastrar el ruido de una etapa 1 considerablemente menos precisa. Aunque la segunda etapa incorpora información adicional de
...
XGBoost independiente extrae por sí mismo la información temporal que la etapa 1 aporta,
y el control de paridad indica que la vía recurrente, aun en paridad informativa, no
alcanza a los modelos de una sola etapa, con el resultado mixto del ámbito completo que la
sección \ref{sec:paridad_lstm} detalla.
...
(\ref{sec:etapa2_xgboost}), del mismo orden de magnitud que la señal que se intenta
...
problema completo desde el principio con toda la matriz predictora. Este patrón —hibridar
puede mejorar sustancialmente a un componente débil sin llegar a igualar a un componente
fuerte de una sola etapa cuando sus conjuntos de información están anidados— no es
exclusivo de este proyecto: es consistente con lo observado de forma sistemática en las
competiciones M4 y M5 \parencite{makridakis2018m4,makridakis2022m5}, donde los métodos
híbridos y de combinación que destacan lo hacen al combinar familias de modelos con
mecanismos e información de entrada genuinamente distintos, como la sección
\ref{sec:mecanismo_ensemble} ilustra para el ensamble de este TFM, y no al encadenar dos
etapas sobre conjuntos anidados. El resultado negativo del híbrido residual es, por tanto,
consistente con esa evidencia y no una anomalía específica de esta implementación.
...
adelantado, con su criterio de éxito fijado antes de observar el resultado, diseñados para
explorar el margen de mejora del híbrido y para establecer un punto de comparación
adicional, respectivamente. Ninguno reabre la conclusión central de la sección anterior ni
constituye una búsqueda de una configuración que la revierta.
...
precisamente en los días de calendario irregular —festivos y puentes, el subconjunto donde
la sección \ref{sec:error_dia} documentó el error relativo más alto del sistema— mejora el
desempeño del híbrido en esos mismos días, a cambio de un coste acotado en el desempeño
global. El híbrido ponderado, descrito en la sección \ref{sec:modelos_control}, asigna un
peso de muestra triple a esas filas (un 8,2\,\% del conjunto residual) sin modificar
ningún otro elemento del modelo. El criterio
...
\begin{table}[htbp]
  ...
  \caption{\headlinecolor{Veredicto del criterio prerregistrado del experimento A (repesado 3x de días irregulares); MAE en viajes por día. CRITERIO NO CUMPLIDO: el presupuesto global se cumplió pero el error en días irregulares NO mejoró.}}
...
566.472 a 585.418. Lo más interesante de este resultado nulo no es que el repesado fallara, sino
\emph{dónde} recayó la mejora que sí se produjo: no en los días que se pretendía
corregir, sino en sábados (mejora del 25\,\%) y en laborables ordinarios (mejora del
5\,\%). Con solamente 45 filas repesadas frente a 161 variables de entrada, el peso
adicional no parece producir un mejor ajuste específicamente en esas filas; en su lugar,
perturba el ajuste global del modelo, y esa perturbación resulta beneficiar, por las
características del conjunto de datos, a los grupos más numerosos en lugar de a los
grupos objetivo. Se reporta como lo que es: un dato sobre la fragilidad del repesado en
muestras pequeñas, no como evidencia a favor ni en contra del esquema de pesos como
estrategia general.

\subsection{Experimento B: ensamble SARIMAX + XGBoost independiente}\label{sec:por_que_ensemble}

El segundo experimento no está sujeto a un criterio de éxito binario: es un punto de
comparación que cualquier evaluación rigurosa de este proyecto debe incluir: ¿existe una
combinación más sencilla que los propios componentes de una sola etapa? La respuesta se
construye sin reentrenamiento adicional, combinando de forma puramente aritmética las
predicciones de SARIMAX y de XGBoost independiente ya calculadas, bajo dos esquemas de
ponderación (tabla \ref{tab:ensemble_pesos}):
...
de esa ganancia se desarrolla en la sección \ref{sec:mecanismo_ensemble}. Los dos esquemas
producen resultados casi idénticos (una diferencia de MAE de test de apenas el 0,03\,\%),
lo que no sorprende dado que los pesos por inverso del MAE, 0,4724 y 0,5276, se sitúan muy
próximos a un reparto igualitario.

El modelo de referencia que esta memoria adopta es, pese a que el ensamble inverso al MAE
obtiene el MAE de test nominalmente más bajo de todo el proyecto, el ensamble 50/50, y
debe constar con exactitud cómo se tomó esa decisión. No existía una regla previa para
elegir entre las dos variantes: el experimento B se prerregistró como punto de
comparación sin criterio de éxito, ambas variantes se calcularon en la misma ejecución
sobre validación y test, y la preferencia por la equiponderada se formuló cuando las dos
cifras de test ya eran conocidas. El conjunto de test no interviene en ningún peso (los
del inverso del MAE proceden solo de validación) y la elección no lo optimiza:
el ensamble 50/50 es la variante con \emph{peor} MAE tanto en validación (227.588
frente a 226.418) como en test (139.725 frente a 139.681). Repetir la selección solo con
validación no la sanearía: ajustar los pesos en validación y evaluar en test no es
circular, es el procedimiento estándar; lo circular sería usar el MAE
de validación para elegir \emph{entre} las dos variantes cuando los pesos de una de ellas,
las del ensamble inverso al MAE, se ajustaron sobre esa misma partición y hacen su cifra de
validación levemente optimista frente a la de la equiponderada. El criterio que sostiene la
elección es, por tanto, el a priori de simplicidad y ausencia de pesos estimados, aplicado
con posterioridad a los resultados; se reconoce como limitación en la sección
\ref{sec:limitaciones}, y la diferencia entre ambas variantes, 44 viajes diarios de MAE
(un 0,03\,\% del mejor resultado), carece de relevancia práctica.

\section{Descorrelación de errores: una explicación compatible con la ganancia del ensamble}\label{sec:mecanismo_ensemble}

Una explicación compatible con que el ensamble supere a ambos componentes, y no solo el
hecho empírico de que lo hace, se obtiene observando directamente la relación entre los
errores de SARIMAX y de XGBoost independiente en cada día de test (figura
\ref{fig:dispersion_errores}).
...
modelos que atacan el mismo fenómeno y comparten, por tanto, una parte de su señal de
error, pero claramente inferior a la unidad. Esa distancia respecto a la correlación
perfecta es la que, según la identidad clásica de combinación, hace ventajoso promediarlos. Para dos pronósticos con varianza de error igual $\sigma^2$ y correlación
...
exactamente el mismo error cuadrático medio. Esta reducción de varianza por promediado de predictores parcialmente descorrelacionados
es, además, el mismo principio que sustenta los comités o \emph{ensembles} de modelos en
el aprendizaje automático en general \parencite{perrone1993networks}, aplicado aquí no a
una colección de redes con la misma arquitectura sino a dos familias de modelos
deliberadamente distintas.

La tabla \ref{tab:desglose_dia_test} explica además por qué esa correlación es
precisamente 0,576 y no un valor más cercano a la unidad: SARIMAX y XGBoost independiente
no cometen simplemente errores de magnitud distinta sobre los mismos días, sino errores
grandes en \emph{días distintos} (el primero en laborables ordinarios comparativamente,
el segundo en fin de semana y festivos), de modo que su correlación de error refleja una
complementariedad estructural de calendario y no solo ruido estadístico independiente. La
ganancia observada, un MAE de test de 139.681 para el ensamble inverso al MAE, un
10,5\,\% por debajo del mejor componente individual (XGBoost independiente, 156.121),
tiene el signo y el orden de magnitud que la identidad anterior anticipa bajo su supuesto
simplificador de varianzas iguales. Los resultados pueden interpretarse a la luz de la
literatura clásica sobre combinación de pronósticos: la combinación puede beneficiarse
cuando los modelos presentan errores parcialmente complementarios o no perfectamente
correlacionados. En este estudio, la correlación observada entre los errores de SARIMAX y
XGBoost proporciona una explicación compatible con la mejora obtenida por el ensamble, sin
que ello constituya por sí mismo una demostración causal del mecanismo.
...
residual. No es una descomposición (LSTM independiente, híbrido y
XGBoost independiente son tres modelos distintos, no sumandos de un mismo error), sino
...
completa entre LSTM independiente (MAE 280.952) y XGBoost independiente (156.121) es de
...
LSTM independiente rinde bastante peor en esa partición), pero los 35.895 viajes
restantes siguen favoreciendo a XGBoost independiente. La conclusión es la misma en ambas
...
XGBoost independiente (244.843) y 138.000 por encima de SARIMAX (273.467), y las tres
...
\subsection{Ablación de bloques de variables sobre XGBoost independiente}\label{sec:ablacion_bloques}

Se reentrena XGBoost independiente sobre subconjuntos restringidos de variables,
...
de todos modos evidencia débil. En conjunto, la respuesta a Q1 es la que sigue. Los
resultados sugieren que las dificultades de la primera etapa no pueden atribuirse
únicamente a su carácter univariante. Los controles realizados apuntan a una limitación
relevante de la arquitectura recurrente en esta serie y con este tamaño muestral, aunque el
resultado mixto obtenido con la matriz completa impide separar de forma definitiva el
efecto de la arquitectura del efecto asociado a la representación y acceso a la
información.
...
XGBoost independiente ya usa de forma medible tanto la información temporal como la
exógena: no hay un componente redundante que la etapa recurrente del híbrido pudiera
aportar por separado. El control de paridad indica que, aun entregándole a una LSTM
...
de la etapa 1, por tanto, no parece explicarse únicamente por el acceso a la
...
XGBoost independiente y que el ensamble, y su $n=5$ lo hace indicativo, no concluyente—.
...
XGBoost independiente por unos 123.000 de MAE.
...
cierra la que quedaba, apoyando por una vía sistemática la respuesta del contraste
central (\ref{sec:contraste_central}).

\section{Respuesta a las preguntas de investigación}\label{sec:respuesta_preguntas}

Se retoman aquí, en el orden de la sección \ref{sec:preguntas_investigacion}, las cinco
preguntas de investigación que estructuran este TFM, respondidas de forma explícita y
remitiendo a la evidencia ya presentada en este capítulo.
...
\begin{description}[nosep,leftmargin=0pt,labelindent=0pt,style=nextline,font=\bfseries]
  ...
  \item[PI2. ¿Bate el híbrido a XGBoost independiente, entrenado sobre las mismas
  ...
  \item[PI3. ¿Reduce el híbrido específicamente el error en festivos y puentes frente a
    SARIMAX y XGBoost independiente?]
  ...
  \item[PI5. ¿Existe una combinación más simple que supere a ambos componentes de una
    sola etapa?]
    Sí: el ensamble 50/50 de SARIMAX y XGBoost independiente, sin ninguna red LSTM ni
    reentrenamiento adicional, alcanza
    un MAE de test de 139.725; el ensamble inverso al MAE, ponderado por el inverso del MAE
    de validación (pesos 0,4724/0,5276), lo reduce ligeramente a 139.681,
...
preguntas nuevas, pero refuerzan las respuestas a PI1, PI2 y PI4 por una vía
independiente: la ablación de bloques (\ref{sec:ablacion_bloques}) indica
mecánicamente que XGBoost independiente ya integra la información temporal y la exógena
