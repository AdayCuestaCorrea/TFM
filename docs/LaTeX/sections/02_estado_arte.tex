...
Los cuatro primeros ejes se leen directamente en el cuadro y no se reproducen aquí uno a
uno; conviene destacar de ellos lo que sitúa a este trabajo fuera del rango de los cinco
estudios de referencia. En \textbf{ámbito} (a) es el único que agrega los cuatro
operadores del CRTM en una serie de sistema completo, frente a una sola línea de autobús
en Monje et al. \cite{monje2022deep}, a una red de metro estación por estación en Cardozo
et al. \cite{cardozo2012spatial} o a series ajenas al transporte en Zhang
\cite{zhang2003hybrid}. En \textbf{horizonte} (b), sus 1.310 días continuos (2023-2026)
...
En \textbf{matriz exógena} (c), ningún otro combina una base meteorológica físicamente
...
perfecto en inferencia, decisión resuelta antes de entrenar ningún modelo. En
\textbf{protocolo de validación} (d) es el único que combina partición cronológica de
frontera única, esquema \emph{walk-forward out-of-fold} de 5 bloques expansivos para la
etapa 1 y pronóstico SARIMAX \emph{walk-forward} de un paso sin reestimación, frente al
holdout simple de Monje et al. o los residuos \emph{in-sample} de Zhang; las 257 pruebas
que fijan las reglas anti-fuga del capítulo \ref{cap:datos_features} (de un total de 470,
Anexo \ref{cap:anexo2}) no tienen equivalente declarado en ninguno de los cinco.
...
ARIMA-red neuronal supera a ambos componentes en las tres series que evalúa, la
...
MAE). Leído a la luz de la condición de Granger desarrollada en la sección
\ref{sec:arquitecturas_hibridas}, y no como una simple discrepancia frente a Zhang, ese
contraste es en sí mismo la aportación: una auditoría con validación \emph{out-of-fold}
estricta de las condiciones bajo las cuales la hibridación secuencial de un componente no
lineal y uno exógeno \emph{no} mejora sobre sus partes, frente a aquellas bajo las cuales
un ensamble paralelo de modelos con información parcialmente descorrelacionada sí lo hace.

Es precisamente la ausencia de un antecedente directo que audite un resultado híbrido
negativo bajo un protocolo igualmente estricto, sobre una serie de demanda de transporte
público real, agregada y multimodal, la que constituye el \emph{research gap} que este TFM
ocupa: no la ausencia de arquitecturas híbridas en la literatura —que existen, y Zhang es
su fundamento teórico—, sino la ausencia de una evaluación de cuándo y por qué fallan
