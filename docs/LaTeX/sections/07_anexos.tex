...
\begin{sloppypar}
Este anexo recoge, para consulta autocontenida, la taxonomía completa de las 161
variables predictoras del modelo, obtenidas mecánicamente mediante
...
temáticos que emplea toda la memoria (el análisis exploratorio del capítulo
\ref{cap:datos_features}, la ingeniería de características de la sección
\ref{sec:ingenieria_features} y el análisis de importancia de variables del capítulo
\ref{cap:resultados}), de modo que cualquier cifra de importancia de variable pueda
contrastarse contra el grupo y el recuento exacto de su familia sin releer el código
fuente.

Toda variable de este catálogo lleva el prefijo \texttt{feat\_}, una convención que no es
estética: el código deriva el conjunto de columnas del modelo a partir de ese prefijo, de
forma que la matriz de entrada nunca se define mediante una lista mantenida a mano que
pudiera desincronizarse silenciosamente del código que construye las variables. Los seis
grupos son, en el orden del cuadro \ref{tab:anexo1_grupos_features}: los siete retardos de
la demanda total
...
(\texttt{feat\_\allowbreak{}\{metro,emt,carretera,cercanias\}\_\allowbreak{}lag\_\{1,2,3,7,14,21,28\}}), presentes pese a que
...
cuatro medias y desviaciones típicas móviles de la demanda total, ventanas de 7 y 28 días,
...
El detalle por variable está en los cuadros de top-15 por ganancia de cada modelo XGBoost
del capítulo \ref{cap:resultados}, y la lista nominal completa de las 161 columnas la
genera \texttt{src/features/build\_features.py}, consultable en el repositorio.
\end{sloppypar}

\chapter{Suite de pruebas automatizadas y guardias anti-fuga}\label{cap:anexo2}

\begin{sloppypar}
La afirmación central de esta memoria —que un resultado negativo bien instrumentado es
una contribución metodológica válida— solo se sostiene si el pipeline que lo produce está
libre de los errores de fuga de información que sistemáticamente producen resultados
optimistas y no reproducibles en la literatura de predicción de series temporales. Este
anexo documenta la arquitectura de verificación que respalda esa afirmación: \textbf{472
pruebas automatizadas}, recolectadas y ejecutadas con \texttt{python -m pytest} desde la
raíz del repositorio con el entorno
\texttt{.\textbackslash venv\textbackslash Scripts\textbackslash python.exe} activado,
de las cuales 257 se acumulan en las fases de modelado (0 a 6b, cuadro
\ref{tab:cronograma_fases}) y el resto se añaden durante la redacción: auditan la capa de
presentación (figuras y cuadros), las tres fases diagnósticas de las secciones
\ref{sec:anatomia_residuo_etapa1}, \ref{sec:paridad_informativa} y
\ref{sec:desglose_subgrupos}, el cuaderno principal de flujo y la coherencia interna del
documento: \texttt{test\_prose\_claims\_match\_artifacts.py} fija contra su artefacto de
origen cada cifra de alta relevancia citada en la prosa —métricas de test, tamaños de
partición, recuentos de filas y variables— junto con la lista completa de ficheros en que
cada una debe aparecer, de modo que una cifra no pueda quedar obsoleta en un capítulo
mientras se actualiza en otro. Tres pruebas más fijan mecánicamente que las tres fases
diagnósticas, puramente explicativas, no introducen ningún modelo nuevo en la tabla
maestra ni escriben artefacto alguno bajo \texttt{models/}.
...
sustancial está diseñada para hacer fallar el pipeline si se reintrodujera, ahora o en una
modificación futura, alguna de las siguientes reglas anti-fuga, de la más general a la más
...
\begin{enumerate}
  ...
  \item \textbf{Ninguna transformación ajustada sobre la serie completa antes de la
    partición.} Las 161 variables se calculan sobre las 1.310 filas antes de partir, y
    ello es seguro porque todas son causales (la fila $t$ solo usa filas anteriores a
    $t$) y ninguna estima un parámetro a partir de los datos; los términos de Fourier y el
    calendario son además funciones deterministas de la fecha, de modo que calcularlos
    sobre la serie completa equivale a calcularlos por partición, lo que fija
    ...
    La única transformación que se ajusta sobre datos es el escalador del punto anterior.
  \item \textbf{Predicciones de la etapa 1 siempre fuera del pliegue de entrenamiento
    (\emph{out-of-fold}, OOF) para alimentar la etapa 2.} Ningún residuo que entrena el XGBoost de la etapa 2
  ...
  \item \textbf{Guardia de fuga del mismo día sobre la suma de operadores.} Puesto
  ...
  \item \textbf{Guardia de fuga del mismo día sobre meteorología.} Ninguna de las 21
  ...
  \item \textbf{El modelo de control XGBoost independiente (\texttt{xgboost\_alone}) entrena sobre las 889
  ...
  \item \textbf{El veredicto del experimento A de sensibilidad se evalúa en código, no
    mediante inspección manual.} El criterio prerregistrado de la sección \ref{sec:experimento_a} (mejora en
  ...
  \item \textbf{El tema visual institucional es un objeto compartido, no una paleta
    ...
    pero cumple para la capa de presentación el mismo papel de guardia estructural.
\end{enumerate}
...
correspondiente sobre un caso mínimo y comprueba que el código \emph{sí} falla; una
guardia que nunca se ha visto obligada a disparar es indistinguible de una que no
existe.
\end{sloppypar}
...
entorno implícita en la memoria de quien ejecutó el pipeline.

\textbf{Repositorio.} Todo el código de este trabajo ---ingesta, ingeniería de
características, modelos, evaluación, cuadernos y las fuentes \LaTeX{} de esta memoria---
es de acceso público en \url{https://github.com/AdayCuestaCorrea/TFM}, y a esa dirección
se refieren siempre las menciones a ``el repositorio'' en estos anexos. Los datos de
partida no se versionan allí: proceden de tres fuentes abiertas cuyo origen, formato y
convenciones de lectura detalla el cuadro \ref{tab:fuentes_datos}, y hay que descargarlos
y colocarlos sin renombrar en \texttt{data/\allowbreak{}raw/} antes del primer comando de
la secuencia siguiente, único punto del pipeline que los lee.
...
Dos de esas versiones documentan una incompatibilidad real y no una preferencia:
\texttt{pyarrow}, porque \texttt{pandas} necesita un motor Parquet explícito, y
\texttt{kaleido} en su rama 1.x y nunca en la 0.2.1, porque esta cuelga de forma
indefinida y sin excepción al exportar figuras bajo \texttt{plotly} 6.x ---un fallo
silencioso, más costoso de diagnosticar que un error de instalación.
...
sembrar no garantiza por sí solo resultados bit a bit idénticos si el motor conserva
reducciones no deterministas, y es la reproducibilidad bit a bit la que este trabajo fijó
como convención.
...
\texttt{pip install -r requirements.txt}. Ejecutados después en este orden desde la raíz
del repositorio, con el entorno virtual activado
(\texttt{.\allowbreak{}\textbackslash{}venv\allowbreak{}\textbackslash{}Scripts%
\allowbreak{}\textbackslash{}Activate.ps1} en PowerShell), regeneran de forma determinista
todos los artefactos citados en esta memoria, sin reentrenar ni repredecir ningún modelo
más allá de lo que cada paso indica explícitamente:
...
\begin{enumerate}[nosep]
  ...
  \item \texttt{python -m src.evaluation.ensemble\_baseline} — experimento B (sección
    \ref{sec:por_que_ensemble}): ensamble SARIMAX + XGBoost independiente, sin reentrenamiento.
  \item \texttt{python -m src.evaluation.sensitivity\_report} — consolida ambos y evalúa
    en código el criterio prerregistrado del experimento A;
  ...
  \item \texttt{python -m src.evaluation.stage1\_residual\_anatomy} — tablas de la
    anatomía del residuo y veredictos de los cuatro criterios prerregistrados.
  ...
  \item \texttt{python -m src.evaluation.subgroup\_breakdown} — sección
    \ref{sec:desglose_subgrupos}: reanálisis por quince subgrupos prerregistrados, con
  ...
  \item \texttt{python -m src.evaluation.export\_figures} — exporta las 34 figuras
    estáticas de \texttt{reports/\allowbreak{}figures/}, fuera de todo núcleo de Jupyter.
  ...
  \item \texttt{python -m pytest} — ejecuta la suite completa de 472 pruebas automatizadas (descrita
...
los cinco elementos de visualización comprometidos en la propuesta inicial: demanda real,
demanda predicha, comparación entre modelos, error de predicción e importancia de
variables del modelo XGBoost. Lee de \texttt{data/\allowbreak{}processed/} y de los modelos
ya entrenados de \texttt{models/}, y no entrena ni predice nada; las figuras estáticas de
esta memoria se exportan aparte, mediante \texttt{export\_figures}, fuera de todo núcleo
de Jupyter.

\textbf{Cuaderno principal de flujo.} El cuaderno
\texttt{notebooks/\allowbreak{}07\_flujo\_completo.ipynb} recorre el trabajo en las seis
etapas del ciclo —carga y comprobación de datos, preparación de variables, partición
...
en propósito y método: aquel visualiza artefactos ya persistidos; este \textbf{reejecuta el
pipeline}, recomputando en vivo la ingesta y sus validaciones, la matriz de 161 variables,
la partición cronológica, los cuatro baselines, el SARIMAX —del que solo se carga el orden
$(2,1,2)\times(0,1,2,7)$ ya seleccionado por AIC— y los dos modelos XGBoost. La etapa 1
LSTM es el único modelo que se carga de artefacto, por una razón metodológica y no de
coste: la etapa 2 debe corregir los residuos del mismo modelo que produjo los resultados
aquí reportados, y reentrenar la red los calcularía contra un modelo distinto del que
documenta esta memoria. Se versiona \emph{con sus salidas guardadas}, de forma que pueda
leerse sin ejecutarlo, y no escribe nada en \texttt{data/} ni en \texttt{models/}.

\textbf{Identidad visual institucional.} El naranja empleado en la memoria es una
...
en \textbf{exactamente dos puntos}, uno por capa: la de figuras en
...
Este anexo declara el uso de herramientas de inteligencia artificial generativa en este
Trabajo Fin de Máster, delimita las tareas en que se emplearon y deja explícito qué
decisiones permanecen, sin excepción, bajo la responsabilidad del autor.
...
periodo abril--septiembre de 2026 \parencite{anthropic2026claude,anthropic2026claudecode}.

Su uso se ha concentrado en cuatro tareas.

\textbf{Implementación del código.} Los módulos de Python del repositorio (ingesta,
ingeniería de características, modelado, evaluación y generación de figuras y tablas), y
la suite de 472 pruebas automatizadas que verifica su comportamiento (Anexo
\ref{cap:anexo2}), se han escrito con asistencia de IA a partir de especificaciones
técnicas definidas por el autor. Las decisiones de arquitectura del pipeline, el protocolo
\emph{out-of-fold}, las reglas anti-fuga, la partición cronológica y el diseño de los
controles diagnósticos fueron fijadas por el autor antes de su implementación y
verificadas contra los resultados obtenidos.
...
\textbf{Búsqueda bibliográfica.} Al inicio del desarrollo el autor empleó asistencia de
IA para localizar de forma expeditiva las referencias del proyecto. Cada ficha fue
revisada después de forma independiente: se comprobó su existencia, se consultó la fuente
primaria y, cuando correspondía, se descargó el documento original. Las referencias de
esta memoria no son citas inventadas ni alucinadas por el modelo; todas están verificadas
contra su fuente.
...
\begin{itemize}[nosep]
  \item Anthropic. (2026a). \emph{Claude} [modelo de lenguaje de gran tamaño].
    \url{https://claude.ai}.
  \item Anthropic. (2026b). \emph{Claude Code} [herramienta de desarrollo de software
...
titulares y los veredictos prerregistrados permanecen íntegros en el capítulo
...
\begin{table}[htbp]
  ...
  \caption{\headlinecolor{Perfil del residuo de la etapa 1 por día de la semana: media, mediana, desviación típica y error absoluto medio, en viajes por día, y media relativa a la $\sigma$ global del residuo (adimensional). Serie primaria (393 días).}}
...
\begin{table}[htbp]
  ...
  \caption{\headlinecolor{Magnitud del residuo de la etapa 1 por régimen de calendario (tipo de día y tipo de festivo); residuo absoluto medio en viajes por día. El residuo se concentra 6,52x en festivos y 2,96x en puentes frente a los laborables ordinarios. La fila ``laborable'' ($n=269$) es la suma de ``laborable, ordinary'' (254) y ``laborable, bridge day'' (15); no es un grupo adicional independiente.}}
...
\begin{table}[htbp]
  ...
  \caption{\headlinecolor{LSTM en paridad informativa por ámbito de variables predictoras (control diagnóstico, 5 semillas, solo validación). MAE de validación mediano, en viajes por día, con banda mín-máx, porcentaje del hueco univariante$\to$\texttt{xgboost\_alone} cerrado, número de ajustes degenerados y \texttt{best\_epoch} mediano. Base univariante: mediana de 5 semillas 416.324; mejor semilla 411.142.}}
...
\begin{table}[htbp]
  ...
  \caption{\headlinecolor{Desglose por subgrupo, partición validación (requisito de réplica). La columna ``Veredicto'' repite el veredicto prerregistrado único por subgrupo; esta tabla solo difiere de la \ref{tab:subgrupos_test} en las columnas de MAE y diferencia.}}
...
\begin{table}[htbp]
  ...
  \caption{\headlinecolor{Diferencia de MAE (mejor híbrido $-$ mejor competidor), en viajes por día, con intervalo de confianza \emph{bootstrap} del 95\,\% en la misma unidad, p bruto y p ajustado por Benjamini-Hochberg ($q=0{,}10$), para la familia inferencial en la partición test. En verano el IC se suprime porque la fracción de réplicas utilizables cae a $0{,}81$; la p se conserva.}}
