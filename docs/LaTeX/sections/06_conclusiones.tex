...
estructural del pipeline, se verifican mediante la suite de tests del Anexo
\ref{cap:anexo2}: la matriz de 161 variables queda construida y verificada, y ninguna de
las guardias anti-fuga ha fallado en ningún momento del proyecto. OE2 aporta además un
resultado que gobierna todo lo que sigue: el SARIMAX seleccionado por rejilla AIC no es un
baseline decorativo, sino uno de los dos modelos de una sola etapa más precisos del
estudio.
...
pronósticos: Granger \cite{granger1989combining} exige que los componentes sean
individualmente subóptimos \emph{y} se apoyen en conjuntos de información distintos, y
esta arquitectura incumple la segunda condición —ambas etapas observan, en última
instancia, el mismo pasado de \texttt{total}— como se argumenta en la sección
\ref{sec:resultado_negativo_teoria}. Las competiciones M4 y M5
\cite{makridakis2018m4,makridakis2022m5} documentan que el patrón —métodos simples y
...
Tampoco debe leerse como una arquitectura sin mérito: PI4 confirma que la etapa 2 cumple
su función, reduciendo el MAE de su propia etapa 1 en un 16,6\,\% en test y un 58\,\% en
festivos, y que su variable de mayor ganancia, \texttt{feat\_day\_type\_festivo}, es
exactamente la señal de calendario que una LSTM univariante no puede ver por construcción
(sección \ref{sec:importancia_variables}). El mecanismo que justifica la hibridación es
real y medible; lo insuficiente es la etapa 1 de la que parte.

El modelo que mejor resuelve el problema no requiere ninguna red neuronal: PI5 confirma
...
componentes. No es un artefacto estadístico sino la identidad de Bates y Granger
\cite{bates1969combination} aplicada a la correlación de 0,576 entre los residuos de ambos
...
segundo en fin de semana y festivos—, que es lo que hace rentable promediarlos.

La lectura conjunta de ambos resultados —el híbrido secuencial que no mejora porque
comparte información entre etapas, y el ensamble paralelo que sí mejora porque combina
información parcialmente descorrelacionada— es la aportación empírica central de este TFM:
...
diferencia de Cardozo et al. \cite{cardozo2012spatial}, que modelan las entradas estación
