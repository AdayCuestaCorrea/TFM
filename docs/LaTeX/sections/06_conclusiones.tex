...
Este capítulo no introduce ninguna cifra nueva: toda afirmación que sigue remite a una
tabla o figura ya presentada y discutida en el capítulo \ref{cap:resultados}, del que es
una síntesis argumental y no una nueva fuente de evidencia.
...
\begin{itemize}
  ...
  \item \textbf{OE4, etapa 2 y controles.} \emph{Qué se hizo:} XGBoost sobre 552 residuos
    ...
    los controles permitieron analizar posibles explicaciones del resultado—, \emph{con
    resultado negativo}. \emph{Limitación:}
...
No es un fallo de ejecución sino un resultado que puede interpretarse a la luz de la
literatura clásica de combinación de pronósticos: \textcite{granger1989combining} señala
que la combinación tiende a beneficiarse cuando los componentes son individualmente
subóptimos y aportan información o errores parcialmente complementarios, y esta
arquitectura ofrece poca complementariedad de esa clase —los conjuntos de información de
sus dos etapas están anidados: la etapa 2 añade calendario y meteorología, pero el pasado
reciente que ve la etapa 1 ya está representado en su matriz—, como argumenta la sección
\ref{sec:resultado_negativo_teoria}, que sitúa
además ese resultado frente a lo observado de forma sistemática en las competiciones M4 y
M5 \parencite{makridakis2018m4,makridakis2022m5}.
...
equiponderada (el ensamble 50/50) y de 139.681 en la ponderada por el inverso del MAE (el
ensamble inverso al MAE), en torno a un 10,5\,\% por debajo del mejor de sus componentes.
Esa ganancia se lee con el mismo marco clásico de combinación de pronósticos
\parencite{bates1969combination}: la correlación de 0,576 entre los residuos
de ambos sobre test (sección \ref{sec:mecanismo_ensemble}) —inferior a la unidad porque
fallan en subconjuntos de días sistemáticamente distintos, el primero en laborables
ordinarios y el segundo en fin de semana y festivos— proporciona una explicación
compatible con la mejora obtenida por el ensamble, sin que ello constituya por sí mismo
una demostración causal del mecanismo.

La lectura conjunta de ambos resultados —el híbrido secuencial que no mejora y cuyas
etapas observan conjuntos de información anidados, y el ensamble paralelo que sí mejora,
en consonancia con la combinación de errores parcialmente descorrelacionados— es la
aportación empírica central de este TFM: una auditoría con protocolo \emph{out-of-fold}
estricto en toda su cadena de las condiciones bajo las cuales cada estrategia de
combinación ha funcionado aquí y bajo cuáles no, sobre una serie real de demanda de
transporte público agregada y multimodal.
...
diferencia de \textcite{cardozo2012spatial}, que modelan las entradas estación por
estación, esta memoria no ofrece resolución espacial alguna: de sus resultados no cabe
...
particularmente escasa precisamente en los subgrupos de calendario más interesantes para
este estudio: la partición de test contiene solo 5 días festivos y 3 laborables con puente
(tabla \ref{tab:desglose_dia_test}). Todas las afirmaciones de esta memoria sobre el
comportamiento de los modelos en esos dos grupos, incluida la mejora del 58\,\% de la
etapa 2 sobre su etapa 1 en festivos, se han etiquetado consistentemente como indicativas
y no concluyentes; la base más sólida para generalizar sobre el comportamiento estructural
frente al calendario sigue siendo el diagnóstico exploratorio de la fase 4 sobre el
periodo completo (48 festivos y 54 puentes; la detección de calendario identifica 55 días
puente sobre los 1.310 días totales, de los que 54 caen fuera de los 28 días de
calentamiento excluidos del entrenamiento), no las particiones reducidas de evaluación
final.

\textbf{Recorte de 28 días por calentamiento de las variables de retardo.} El retardo y
la media móvil de mayor ventana (28 días) obligan a descartar los 28 primeros días de la
serie de entrenamiento, que queda reducida de 917 a 889 días efectivos; la política no
alcanza nunca las particiones de validación o test (sección \ref{sec:particiones_fuga}).
Es un coste aceptado y documentado, pero real, sobre el volumen de datos disponible para
el ajuste de los modelos.

\textbf{Naturaleza provisional de los registros de demanda del CRTM.} El propio fichero
fuente (\texttt{CRTM\_Evolucion\_demanda\_diaria.xlsx}) advierte explícitamente que sus
cifras son provisionales y sujetas a revisión posterior por el consorcio. Este TFM no
tiene control sobre esa advertencia ni forma de cuantificar la magnitud de una eventual
revisión futura; toda cifra de demanda y todo error de modelo reportado en esta memoria
hereda, por tanto, la incertidumbre de la fuente original.
...
pronóstico atmosférico perfecto que no existe en producción. La decisión es conservadora
y correcta para el objetivo del trabajo, pero implica que ninguna cifra de esta memoria
estima la mejora alcanzable con un pronóstico meteorológico operativo de calidad: esa
cota queda fuera de alcance y se retoma como línea futura.
...
espectrales, individualmente inconcluyentes. El hallazgo central de esa sección (que el
residuo retiene estructura de \emph{calendario}, no de clima) no se ve afectado, pero la
caracterización de su componente temporal fino es más matizada de lo que un veredicto
binario sugeriría.
...
(objeción O6). La conclusión de la sección —que las dificultades de la etapa 1 no pueden
atribuirse únicamente a su carácter univariante y apuntan a una limitación relevante de la
arquitectura recurrente en esta serie y con este tamaño muestral— se apoya en el ámbito
...
No existía una regla previa para preferir el ensamble 50/50 al ensamble inverso al MAE:
ambas se calcularon en la misma ejecución y la preferencia se formuló con las dos cifras
de test a la vista. El test no interviene en ningún peso y la elección no lo optimiza —la
equiponderada es la peor de las dos tanto en validación como en test—, pero la decisión
de presentación se tomó a posteriori y se registra como tal. El procedimiento exacto, y
por qué rehacer la selección únicamente con validación tampoco la sanearía, constan en la
sección \ref{sec:por_que_ensemble}. La diferencia entre ambas, 44 viajes diarios de MAE,
...
Las siguientes líneas se enuncian como extensiones metodológicas concretas y
justificadas por los propios resultados de este TFM; ninguna ha sido ejecutada ni
evaluada en este proyecto.
...
independiente para cada operador, lo que permitiría dos preguntas que este TFM no puede
responder: si la ventaja del ensamble sobre el híbrido se mantiene a una escala de
demanda menor y con patrones de calendario específicos de cada modo —el perfil de
Cercanías, más ligado a la movilidad laboral pendular, es previsiblemente distinto del de
la red de EMT—, y si un modelo conjunto multivariante que comparta representación entre
operadores aporta algo que cuatro modelos independientes no capturan.

\textbf{Integración de pronósticos meteorológicos operativos.} Como señala la limitación
correspondiente, este TFM emplea exclusivamente meteorología retardada para evitar
asumir un pronóstico perfecto. Una extensión natural, y metodológicamente sólida, sería
sustituir esos rezagos por pronósticos meteorológicos reales de horizonte 1-7 días de un
proveedor operativo (por ejemplo, la API de pronóstico de Open-Meteo, complementaria a su
API histórica ya empleada en este proyecto), evaluando el sistema bajo la incertidumbre
de pronóstico real en lugar de bajo el valor observado retardado. Esto permitiría
cuantificar, con evidencia y no por extrapolación, la brecha entre el límite superior
bajo pronóstico perfecto y el desempeño alcanzable en un escenario de producción
genuino.
...
y anual simultáneamente— sin la memoria secuencial de una red recurrente clásica; si
sobre esta serie lo consiguen es una cuestión empírica abierta. Sustituir la etapa 1 por
una arquitectura de este tipo, entrenada bajo el mismo protocolo estrictamente
\emph{out-of-fold} que gobierna todo este proyecto, permitiría comprobar si una etapa 1
más expresiva cierra la brecha frente a SARIMAX que este TFM documenta como su hallazgo
central, sin renunciar al rigor de validación que hace de esa comprobación un resultado
...
Sin introducir evidencia nueva, los resultados del capítulo \ref{cap:resultados} admiten
una lectura orientada a la explotación operativa, destino último que motivó este TFM. El
ensamble seleccionado predice la demanda diaria agregada de la red con un MAPE de test del
\textbf{3,13\,\%} (cuadro \ref{tab:comparacion_test}), un margen compatible con los usos
de planificación en los que la unidad de decisión es la red completa y el horizonte es el
día siguiente, el único evaluado en este trabajo: dimensionamiento de la oferta de
material y de turnos de personal, previsión de carga agregada para la programación de
frecuencias, y detección temprana de desviaciones frente a lo esperado. Para esos usos,
la recomendación es doble y deliberadamente austera: emplear la combinación aritmética de
SARIMAX y XGBoost independiente de la sección \ref{sec:por_que_ensemble}, y no una
arquitectura recurrente ---el capítulo \ref{cap:resultados} muestra que la etapa LSTM no
solo no aporta precisión, sino que su coste de entrenamiento, ajuste y mantenimiento no se
traduce en ninguna ventaja medible---; y fijar los pesos a partes iguales, porque la
variante ponderada no mejora de forma apreciable y sí añade una dependencia de la
partición de validación que habría que rehacer con cada reentrenamiento. Es una
recomendación operativa, no un resultado confirmatorio: la preferencia entre ambas
variantes se formuló a posteriori (sección \ref{sec:por_que_ensemble}).

Esa recomendación tiene límites que deben acompañarla siempre, y son exactamente los de
la sección \ref{sec:limitaciones}. El error no se reparte de forma uniforme: se concentra
en los días de calendario irregular ---festivos y puentes (cuadro
\ref{tab:desglose_dia_test})---, precisamente aquellos en los que una decisión de oferta
equivocada resulta más costosa, de modo que las cifras de error agregadas no deben
...
corredor o estación. Y su desempeño es el alcanzable sin pronóstico meteorológico, por la
exclusión deliberada de la meteorología del mismo día (sección \ref{sec:limitaciones}):
incorporarlo es la vía de mejora inmediata y la segunda línea de la sección
...
de viajes diarios, el modo de desplazamiento que la meta 11.2 busca hacer accesible,
asequible y sostenible. La predicción a un día alimenta las decisiones de
dimensionamiento de flota, turnos de personal y programación de frecuencias de la sección
\ref{sec:implicaciones_planificacion}: una previsión más ajustada permite sostener el
nivel de servicio sin sobredimensionar la oferta, que es como un sistema de transporte
público mejora su acceso sin aumentar su coste. La contribución es indirecta ---el
trabajo no despliega ningún sistema--- y está acotada exactamente por la sección
\ref{sec:limitaciones}: no hay resolución espacial por debajo del agregado de red y el
error se concentra en el calendario irregular, los días en los que una decisión de oferta
...
modelos ya entrenados, sin entrenamiento adicional ni hardware especializado. En un campo
cuya respuesta por defecto ante un problema de predicción es escalar la arquitectura, un
resultado negativo bien documentado ahorra ese gasto computacional a quien venga después,
y ese ahorro sí es directamente atribuible al trabajo. No se cuantifica aquí ninguna reducción de emisiones ni se afirma relación
