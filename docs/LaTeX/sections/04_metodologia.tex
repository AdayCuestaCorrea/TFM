...
demanda, y un modelo XGBoost (etapa 2) que corrige, a partir de variables exógenas de
calendario y meteorología, el residuo que la etapa 1 deja sin explicar. Formalmente, si
...
Zhang \cite{zhang2003hybrid}, que es su referente más directo en la literatura y al que
...
sobre sus propios retardos, sino como una función de 161 variables exógenas (calendario,
meteorología retardada, términos de Fourier) construidas de forma independiente de la
...
Antes de evaluar cualquier arquitectura de aprendizaje automático es necesario fijar el
listón frente al que se mide: un conjunto de modelos de referencia sencillos que
establezcan cuánta señal puede explicarse sin recurrir a redes neuronales ni a variables
exógenas. El primero, la persistencia $\hat y_t = y_{t-1}$, resulta ser un hombre de paja
...
superar a la persistencia estacional, y no a la persistencia simple, es la barra honesta
...
ajuste adicional y contaminaría la comparación honesta que se plantea en el capítulo
...
estacional de la serie \cite{box1970time}, y añadir retardos hechos a mano expresaría la
...
condiciona exclusivamente sobre las observaciones reales hasta $t-1$. Se prefiere este
...
La primera etapa del híbrido es una red LSTM \cite{hochreiter1997lstm}
...
Conviene señalar, con la misma transparencia con la que se presentan las demás
decisiones metodológicas de este capítulo, que $W$ se eligió observando precisamente la
...
una capa LSTM de 32 unidades con \emph{dropout} de 0,2, seguida de una capa densa de
...
Esto no constituye un defecto de la implementación sino la respuesta honesta a la
pregunta que la etapa 1 plantea: la dinámica temporal univariante, por sí sola, no basta
...
\cite{bergmeir2012cv,cerqueira2020evaluating}, adaptada aquí a la generación de
predicciones de \emph{stacking} \cite{wolpert1992stacking} en dos etapas.
...
\begin{table}[htbp]
  ...
  \caption{\headlinecolor{Métricas por fold del esquema OOF walk-forward expansivo (5 folds).}}
  \label{tab:folds_oof}
\end{table}
...
\begin{figure}[htbp]
  ...
  \caption{\headlinecolor{Predicción LSTM fuera de muestra (OOF) por fold del walk-forward expansivo.}}
  \label{fig:oof_folds}
\end{figure}
...
2024-01-01 hasta el 2025-07-05, con las 161 variables exógenas de la sección
...
XGBoost \cite{chen2016xgboost} se entrena sobre este conjunto residual con los
...
\begin{table}[htbp]
  ...
  \caption{\headlinecolor{Hiperparámetros seleccionados por grid search de 54 puntos (idéntico para ambos modelos XGBoost) sobre un holdout interno cronológico.}}
...
tabla \ref{tab:top15_residual}, capítulo \ref{cap:resultados}), lo que confirma
empíricamente que la etapa 2 sí aprende, a partir de las variables exógenas, precisamente
la señal de calendario que la etapa 1 no puede ver por diseño: el mecanismo que motiva la
arquitectura híbrida es real y medible, con independencia de que el resultado agregado del
...
de la familia de aprendizaje automático tabular (\emph{random forest}). Se ha prescindido
...
matriz exógena completa), que sí discriminan entre las explicaciones rivales del
...
El segundo control de la sección anterior, SARIMAX, y el modelo \texttt{xgboost\_alone}
...
SARIMAX y \texttt{xgboost\_alone} \emph{sí} satisfacen esa condición: el primero modela
explícitamente la autorregresión y la estacionalidad semanal mediante su componente
$(p,d,q)\times(P,D,Q,s)$, mientras que el segundo aprende relaciones no lineales
directamente sobre variables exógenas de calendario y meteorología, sin ningún término
autorregresivo. El híbrido secuencial de este TFM, en cambio, la \emph{incumple}, porque
ambas etapas observan en el fondo el mismo pasado de la serie \texttt{total} (sección
...
\texttt{xgboost\_alone} y se contrasta la reducción de varianza observada frente a la
que la identidad de Bates y Granger predice; se adelanta únicamente que ese contraste
