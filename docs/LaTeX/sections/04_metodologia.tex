...
entrenamiento se generan siempre fuera del pliegue de entrenamiento (\emph{out-of-fold},
OOF), mediante un
...
El resultado es coherente con la premisa de la arquitectura híbrida: el error absoluto medio de
...
LSTM —LSTM independiente, híbrido e híbrido ponderado; \texttt{lstm\_alone},
\texttt{hybrid} y \texttt{hybrid\_weighted} en los cuadros y el código— son levemente
...
estacional cuando ambas se comparan sobre las mismas filas fuera del pliegue de
entrenamiento (OOF) generadas por el esquema de la sección \ref{sec:esquema_oof} (MAE OOF de 690.460 frente a 461.087).
...
fuera del pliegue de entrenamiento (\emph{out-of-fold}, OOF): cada predicción procede de
un modelo cuyo entrenamiento no incluyó esa observación. La alternativa sería ajustar un
único modelo LSTM sobre todo el
...
predicción OOF para series temporales sigue la práctica establecida en la
...
Concretamente, se emplea una validación temporal progresiva (\emph{walk-forward}),
en forma de validación cruzada expansiva hacia adelante, de cinco folds sobre el periodo de entrenamiento posterior al
...
información de un bloque posterior. Dentro de cada bloque, la ventana de entrada para el
...
separar la cola de parada anticipada), sus predicciones OOF presentan una
...
predicciones OOF de la sección anterior; su signo se fija de modo que la
...
denominado XGBoost independiente, entrena un modelo XGBoost directamente sobre la
...
es deliberada: XGBoost independiente se entrena sobre las 889 filas completas del
conjunto de entrenamiento posterior al calentamiento, no sobre las 552 filas del conjunto
residual de la etapa 2. La razón es que XGBoost independiente no depende en ningún
...
híbrido ponderado, que no constituye un modelo nuevo, sino una reponderación del
...
SARIMAX (sección \ref{sec:baselines}) y el control XGBoost independiente de la sección anterior
...
SARIMAX y XGBoost independiente \emph{sí} satisfacen esa condición: sus conjuntos de
...
ponderación. La primera, el ensamble 50/50 (\texttt{ensemble\_equal} en los cuadros y el código), fija $w_1 = w_2 = 0{,}5$: no requiere
ningún dato adicional para determinarse y actúa como el punto de referencia neutro de
la combinación. La segunda, el ensamble inverso al MAE (\texttt{ensemble\_inverse\_mae} en los cuadros y el código), pondera cada componente en
...
XGBoost independiente y se pone la ganancia observada en relación con la reducción de
varianza que la identidad de Bates y Granger anticipa; se adelanta únicamente que ese contraste
sitúa al ensamble 50/50 y al ensamble inverso al MAE como los dos modelos
