...
**Supervisor formal review, phase 1 of 5 — APA 7 citation system (items 1-4, 35): complete.**
470/470 tests (no test added; the pending-
verification test was inverted in place). `\bibliographystyle{plain}` → `biblatex`
`style=apa` + `biber` (both verified installed; trial-compiled in a scratch copy before
touching the repo). 36 citation sites converted by hand (22 `\textcite`, 14 `\parencite`);
`\printbibliography[heading=bibintoc,title={Referencias}]` — the list had no TOC entry
before. Bibliography audited entry by entry: every DOI/URL supplied by the author from the
publisher's record, none inferred. **Findings recorded in REPORT_MAPPING incidencia 21:**
`monje2022deep` carried two wrong author first names ("Laura"/"Ricardo" vs the paper's
Leticia Monje / Ramón A. Carrasco — corrected from the local PDF, `pages`→`eid`);
`makridakis2022m5` pages 1325-1336 → 1346-1364; Surribas-Sayago rebuilt as a full
`@inproceedings` from the supervisor's data (no `editor` yet). The "[ficha sin verificar]"
/ "No verificado" strings live in `LITERATURE_COMPARISON` (generator), not the `.tex`.
The two "No verificado" cells of the Surribas row (horizonte, protocolo) were filled from
the paper itself (§3.1 Experimental Setup: 1-min DIF at CESAR, December 2023; chronological
80/20 holdout, single run, no CV) and the table regenerated by the generator, never by hand.
Length measured, not estimated: body 73 → 73 pages, C1 36,255 → 36,309. `audit_guia_docente.py`
now accepts "Referencias" as the list heading and drops `\textcite`/`\parencite` in C5.

**Supervisor formal review, phase 2 of 5 — state of the art (items 5-7): complete.**
470/470 tests (the literature-table shape test was rewritten in place, not added). Three new
references, every field verified by the author against the publisher's record and every
table cell read from the paper itself: `yazicioglu2025passenger` (Istanbul bus+rail, hourly
smart-card boardings, XGBoost gain-based feature selection whose ranking is led by hour,
school day, weekday, holiday and month with precipitation and snow among the last — an
independent replication of the memoria's calendar-dominant result, which §2.1 previously
asserted citing only chapter 5), `arana2014weather` (Gipuzkoa smart cards, wind and rain
reduce trips; the circulating `j.trb` DOI is wrong, the Crossref-registered `j.tra` one is
used) and `chen2019emd` (EMD-LSTM on one Chengdu metro station beating LSTM/BPN/ARIMA on a
single test Friday, no exogenous inputs — the hybrid-in-transport antecedent the gap
statement needed) and `singhal2014weather` (NYC subway, daily *and* hourly ridership on the
same data, explanatory OLS without a temporal split — the one reviewed work that compares
both scales, supporting daily aggregation as a design decision; its row waited until the
author confirmed the five cells, since the paper could not be read from this session and
nothing is inferred). Chen's author given names were first drafted from memory, checked
against Crossref before use, and four of seven were wrong — recorded in incidencia 22 as
the second citation-accuracy defect of this review after Monje. Table 2.1 grows 6 → 10 rows
in the generator. Item 6: §2.3 now states
explicitly that Surribas-Sayago is a methodological reference and *not* an antecedent of
public transport demand forecasting, and drops the dangling "documento de contexto técnico"
appeal. Item 7: seven absolute claims reformulated, all in chapter 2, the research gap with
the supervisor's literal wording; the whole of `sections/` swept, four remaining hits
documented as internal (not bibliographic). Length: body 73 → 75 on first compile, trimmed
back to **73**; C1 36,309 → 36,876. Details: REPORT_MAPPING incidencia 22.

**Supervisor formal review, phase 3 of 5 — methodological precision (items 13-21 + §6.3):
complete.** 470/470 tests (no test added or removed). The only phase of the review that
changes what the memoria *claims*; every edit was a class sweep (25 "exógenas" renamed and 16
kept on purpose, 9 "same information set" sites, 8 "stage 2 sees more" sites, 8 Bates &
Granger sites) and closed with a claim-by-claim cross-read over resumen → abstract → §2.3 →
§4.7 → §5.6.1 → §5.10.3 → §5.12 → §6.1 → §6.2. Item 16 was resolved as *nested, not
identical* information sets (stage 2 adds the exogenous block but its lags/rolling stats
already represent the 28-day window stage 1 sees), which keeps the Granger backbone and the
PI4 mechanism claim both true; the supervisor's empirical sentence is added on top. The
nested-set consequence is **not** attributed to Granger (1989) — stated as a logical
consequence about this matrix; the author may later supply a *forecast encompassing* source.
Item 21: the 2/11 reasoning leaves the prose and `main()`; `expected_first_by_chance` and
its test stay (docstring rewritten) so the count holds at 470. §6.3 reformulated without a
new citation. **Two accuracy defects found by the sweep, recorded as findings (REPORT_MAPPING
incidencia 23e/23f, third and fourth of the review after Monje and Chen):** §4.7 claimed
`xgboost_alone` has "ningún término autorregresivo" (it has 7 lags + 4 rolling stats; §5.10.1
measures that block at +30.9 %), and §5.8/§5.7.2 argued the ensemble gain exceeded what
*uncorrelated* errors would explain — inverted, since ρ=0 halves the variance and ρ=0.576
leaves 79 %. Measured: body **73 → 74** (accepted, no approved content cut), 97 pages, 0
errors/undefined, biber 0 warnings, 0 margin violations, resumen 469 / abstract 480 words
each on one page (the added ~25 words had pushed both keyword blocks to a second page;
recovered by `\vspace` 0.5 → 0.2 cm and ~30 connective words), C1 36,876 → 37,492 (+616).
PI1–PI5 literal-identical between §1.2 and §5.12 (scripted check).

**Supervisor formal review, phase 4 of 5 — language, register and terminology (items
22-30): complete.** 470/470 tests (none added or removed; two column-name strings updated
in `test_export_latex_tables.py`). Changes how the memoria reads, not what it claims; every
item swept by class over `sections/` and the generator literals (165 replacements applied
by an engine that aborts on any count mismatch). Item 23 incl. the OE1 heading; item 24
reverses the 13f decision on the supervisor's instruction (both spellings had been
coexisting, 7+9); items 25-26: 38-term inventory, introduce-once-then-keep for the
technical anglicisms, "tests" → "pruebas automatizadas" for the suite but "contraste" for
the four statistical-sense sites; item 27: technical identifier (unchanged, 57×
`\texttt{xgboost\_alone}`, tables, `PROSE_CLAIMS` which pins numbers not names) vs prose
name ("XGBoost independiente" replaces three coexisting variants; generator strings and
one axis title follow); item 28: the `R2` was the subplot label of figure 5.1
(`dashboard_figures.py`), not prose; item 29: 33 sites; item 30: rule = categorical stays
for measured counts / pre-registered code-evaluated verdicts / test-enforced guarantees,
softened for interpretations from rankings, with the kept list recorded. **Fifth
accuracy/consistency finding of the review (REPORT_MAPPING 24i):** §4.4/§5.5/PI4/§6.1
"confirmed" the stage-2 calendar mechanism with an impurity-gain share, the very metric
§5.10.3 rules out as evidence of predictive utility (weather block: 51.6 % of gain, net
non-contributor); the gain-based half is softened, the measured half (−16.6 % / −58 %
MAE) stays categorical. Figures re-exported: exactly the 3 expected PNGs changed of 34;
tables: exactly the 6 expected of 32. Measured: body **74 → 74**, 97 pages, 0
errors/undefined, biber 0 warnings, 0 margin violations, resumen 468 / abstract 480 words
on one page each, C1 37,492 → **37,483** (−9), C5 28,928 → 28,898. PI1–PI5 identical
between §1.2 and §5.12 (scripted). Retired-formulation grep 0 in `sections/`.

**Supervisor formal review, phase 5 of 5 — final verification pass: complete.** 470/470
tests (none added or removed). A phase with **no planned changes**: it establishes that the
four rounds left the memoria correct, internally consistent and consistent with the code
and the published tree; every edit it made is a defect fixed, not an improvement. Gate 0
first: Phase 4 had never been committed (HEAD was the Phase-3 commit, the working tree
carried Phase 4's exact footprint) — committed on its own (`f062c71`) so history stays
phase-by-phase. **Regression against the items the supervisor approved (8-12, 18, 19,
31-34): none altered** — every `git diff 6f9e162 HEAD` hunk touching those passages is a
recorded Phase 1-4 edit (citation form, item 14/22/25/29/30 wording); items 8 and 34 have
no hunk at all. Cross-phase: retired formulations 0 in `sections/` (two deliberate keeps);
the three Phase-3 formulations read consistently across the nine cross-read sites;
`\textcite` 30 / `\parencite` 13 / bare `\cite` 0 / numeric 0; bibliography 23 ↔ 23 both
ways; Phase 2's four references cited and present; tables and figures regenerated
**byte-identical** (`git status` empty). Coherence: `PROSE_CLAIMS` 17/17; the numeric-token
diff of the prose since the reviewed commit (6 removed, 17 added) is explained token by
token by recorded edits — no measured value moved; every `\ref` resolves, every float
referenced, numbering gap-free (28 figures, 33 tables); PI1-PI5 identical; 198 `\texttt{}`
paths/identifiers all resolve. **Eight defects fixed** (REPORT_MAPPING 25f), two of them
accuracy findings recorded as such: `05:847` claimed 411.142 is cited in §5.12 and ch. 4
(false — only in the Anexo E caption; stale since the reduction pass), and `02:141`
attributed an LSTM citation (Hochreiter & Schmidhuber) to Zhang (2003)'s neural net, which
is an MLP — it survived as a bare numeric `\cite` that Phase 1 converted in place. With
these, **seven accuracy defects** surfaced by the review, none visible in the composed
text. Also fixed: a Phase-4 gender agreement (`07:81`), one missed "automáticas" (`07:368`),
"la primera" → "la segunda" line of future work (`06:226`), a wrong `\ref` in Anexo C
(`07:273`), "Surribas Sayago" → "Surribas-Sayago" in resumen/abstract (portada untouched),
and a false "segundo control de la sección anterior, SARIMAX" (`04:435`). Left on purpose:
the resumen/abstract sentence asymmetry (adding it pushes the keywords to page 2, 23i) and
the two "documento de contexto técnico" references. Measured on the final tree: 97 pages,
body **74**, 0 errors/undefined, biber 0 warnings, 0 margin violations, resumen 467 /
abstract 479 words on one page each, C1 37.483 → **37.467**. Public repository: now answers
HTTP 200 unauthenticated but sits at `92c000e` (pre-review); exclusion set re-derived and
